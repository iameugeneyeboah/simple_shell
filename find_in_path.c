...
int find_program(data_of_program *data)
{
	...
	{/* appends the function_name to path */
		...
		{/* IF the filE is found, is not a directory and has execute permisions*/
			errno = 0;
			free(data->tokens[0]);
...
char **tokenize_path(data_of_program *data)
{
	...
	/* check the number of directories in that PATH */
	for (i = 0; PATH[i]; i++)
	{
		if (PATH[i] == ':')
			counter_directories++;
...
/**
 * check_file - checks if there exists a file
 * if it is not a directory and
